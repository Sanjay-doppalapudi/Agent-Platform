...
- **Sandboxing hardening + container mode** (2026-08): opt-in `sandbox:"container"` (bash in docker/podman, workspace-only mount, egress off); configurable `network` egress policy (`deny`/allowlist) across fetch/websearch/bash; workspace-trust store hard-deny; privileged-config write deny (`ap.config.json`/`.mcp.json`/`.git/hooks`); Windows trailing-dot/space filename normalization; the REPL `!` shell-escape.
...
- **OS-level sandboxing** (Codex Seatbelt/Landlock): SHIPPED as an opt-in **container mode** (2026-08) rather than platform-native code. `sandbox:"container"` (`--sandbox container`) runs each `bash` inside a throwaway docker/podman container — workspace-only bind-mount, `--network none` by default, `--security-opt no-new-privileges` — a real OS boundary that contains the escapes the in-process guardrail cannot (a determined model can decode a path at runtime and escape the pattern scan). File tools stay in-process (already contained). A cross-platform container is the portable answer; Landlock/Seatbelt/bwrap remain possible progressive enhancements. Paired with a configurable `network` egress policy (`deny` / hostname allowlist) across `fetch`/`websearch`/`bash`.
