...
- **`websearch` tool**: web search via DuckDuckGo's HTML endpoint (plain fetch, no API key) — titles, URLs, snippets. **`fetch` tool**: URL → readable text (50KB cap). Browser rendering is temporarily disabled because a system browser cannot enforce AP's network policy across DNS, socket connections, and redirects. **`todo` tool**: session checklist rendered live in the transcript.
